...
STATIC_FUNC void sprd_dolphin_nand_ecc_layout_gen(struct sprd_nand_oob *oob, uint8_t sector_num, struct nand_ecclayout *layout)
{
	...
	for(sct = 0; sct < sector_num; sct++)
	{
		...
		layout->oobfree[sct].offset = oob->nOOBSize * sct + oob->nInfoPos;
...
STATIC_FUNC int sprd_dolphin_nand_read_lp(struct mtd_info *mtd,uint8_t *mbuf, uint8_t *sbuf,uint32_t raw)
{
	...
	cfg2 = (dolphin->ecc_mode << ECC_MODE_OFFSET) | (dolphin->info_pos << SPAR_INFO_POS_OFFSET) | ((dolphin->sct_pg - 1) << SPAR_SECTOR_NUM_OFFSET) | dolphin->ecc_pos;
...
STATIC_FUNC int sprd_nand_probe(struct platform_device *pdev)
{
	...
	/*turn on the emmc power, otherwise may cause  I/O error in Low and high temperature test*/
	/*only for 7731gga*/
	mmc_regu = regulator_get(NULL,"vddemmcio");
	regulator_enable(mmc_regu); 
